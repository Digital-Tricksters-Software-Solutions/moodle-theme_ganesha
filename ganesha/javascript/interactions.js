/**
 * Ganesha Theme Playful K-12 Interactions (Non-AMD Vanilla Version).
 * Includes Sensory Mode toggle, Canvas Confetti triggers, and Password Peek-a-boo.
 *
...
(function() {
    ...
    // Main initializer run on DOM load
    function initInteractions() {
        // Create dynamic floating sensory Cozy Mode button if not present in the DOM
        if (!document.querySelector('.sensory-toggle-btn')) {
            ...
            floatingBtn.innerHTML = '✨ Sensory Cozy Mode';
        ...
        function applySensoryMode(mode) {
            if (mode === 'cozy') {
                ...
                toggleBtns.forEach(function(btn) {
                    ...
                    btn.innerHTML = '🌸 Enable Sparkles';
                });
            } else {
                body.classList.remove('sensory-cozy');
                toggleBtns.forEach(function(btn) {
                    btn.classList.remove('cozy');
                    btn.innerHTML = '✨ Sensory Cozy Mode';
                });
            }
        }
        applySensoryMode(currentMode);

        // Sensory Toggle Event Delegation (vanilla)
        ...
        // 2. Confetti on Task Complete (vanilla)
        document.addEventListener('click', function(e) {
        ...
        // --- 4. REAL SYNTHESIZED CHIPTUNE RADIO ENGINE (Web Audio API) ---
        var ChiptuneRadio = {
            audioCtx: null,
            currentSource: null,
            isPlaying: false,
        ...
        // Expose Chiptune Radio globally
        window.GaneshaChiptune = ChiptuneRadio;

        // --- 5. INTERACTIVE WEATHER SKY BOARD CONTROLLER ---
        var SkyWeather = {
        ...
        // Parse raw settings variables set by Moodle PHP
        var rewardsList = [];
        if (window.GaneshaRewardsRaw) {
            rewardsList = window.GaneshaRewardsRaw.split(',').map(function(item) {
        ...
        var triviaList = [];
        if (window.GaneshaTriviaRaw) {
            triviaList = window.GaneshaTriviaRaw.split('\n').map(function(l) { return l.trim(); }).filter(function(l) { return l.length > 0; });
        }
        ...
        var petSpeeches = [];
        if (window.GaneshaSpeechesRaw) {
            petSpeeches = window.GaneshaSpeechesRaw.split('\n').map(function(l) { return l.trim(); }).filter(function(l) { return l.length > 0; });
        }
        ...
        var owlMascot = window.GaneshaOwlMascot || "🦉";
        var petMascotVal = window.GaneshaPetMascot || "🐘";
        ...
        // --- 6. INTERACTIVE DAILY REWARD SCRATCH CARD CONTROLLER ---
        var ScratchCard = {
            canvas: null,
            ctx: null,
            isDrawing: false,
            ...
            reset: function() {
                ...
                this.ctx.fillText('⭐ SCRATCH HERE ⭐', this.canvas.width / 2, this.canvas.height / 2 + 5);
            },
        ...
        // --- 7. INTERACTIVE SPEECH OWL ENGINE ---
        var OwlController = {
            factIndex: 0,
            cycleFact: function(e) {
                ...
                if (rawText.indexOf("Did you know?") === 0) {
                    textEl.innerHTML = '<strong>Did you know?</strong>' + rawText.substring(13);
                } else {
        ...
        // --- 8. GANESHA VIRTUAL PET COMPANION ENGINE ---
        var PetController = {
            level: 1,
            exp: 0,
            ...
            feed: function(e) {
                ...
                if (levelUp) {
                    bubbleEl.innerHTML = '"🎉 Friendship Level Up! We are now level ' + this.level + '! You are my best friend! 🐘❤️"';
                    if (window.GaneshaChiptune) {
                        setTimeout(function() {
